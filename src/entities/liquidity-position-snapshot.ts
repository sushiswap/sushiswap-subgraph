...
export function createLiquidityPositionSnapshot(position: LiquidityPosition, block: ethereum.Block): void {
  ...
  snapshot.lca = position.lca
  snapshot.lcad = position.lcad
  snapshot.liquidityTokenBurned = position.liquidityTokenBurned
  snapshot.liquidityTokenMinted = position.liquidityTokenMinted
  snapshot.burnCount = position.burnCount
  snapshot.mintCount = position.mintCount
  snapshot.lastBurn = position.lastBurn
  snapshot.lastMint = position.lastMint
  snapshot.save()
}
