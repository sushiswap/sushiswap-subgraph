...
export function deposit(event: Deposit): void {
  ...
  if (event.params.amount.gt(BIG_INT_ZERO)) {
    ...
    if (!reservesResult.reverted) {
      ...
      const token0Amount = reservesResult.value.value0
        .div(totalSupply)
        .times(event.params.amount)
        .divDecimal(BIG_DECIMAL_1E18)
      const token1Amount = reservesResult.value.value1
        .div(totalSupply)
        .times(event.params.amount)
        .divDecimal(BIG_DECIMAL_1E18)
      const entryUSD = token0Amount
        .times(getUSDRate(pairContract.token0(), event.block))
        .plus(token1Amount.times(getUSDRate(pairContract.token1(), event.block)))
